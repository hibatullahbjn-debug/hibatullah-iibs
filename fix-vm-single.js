...
const startTag = '<div class="sdih-visi-card">';
const endTag = '</div>\r\n    </div>\r\n  </section>'; // This includes the end of sdih-misi-grid

let startIndex = html.indexOf(startTag);
// Find the end of sdih-misi-grid. Let's find <!-- NILAI-NILAI SECTION -->
...
// Remove the intermediate headers for Visi and Misi
const labelVisi = html.indexOf('<div class="sdih-sec-label"><span></span> VISI SEKOLAH <span></span></div>');
if (labelVisi !== -1) {
  // Let's just keep the Visi/Misi section title above the card.
  // We can change it to "Visi & Misi SMP Hibatullah"
  html = html.replace('<div class="sdih-sec-label"><span></span> VISI SEKOLAH <span></span></div>\r\n      <h2 class="sdih-sec-title">Visi <span>SMP Hibatullah</span></h2>\r\n      <p class="sdih-sec-sub">Cita-cita besar yang menjadi arah dan tujuan pendidikan kami</p>',
  '<div class="sdih-sec-label"><span></span> VISI & MISI <span></span></div>\r\n      <h2 class="sdih-sec-title">Visi & Misi <span>SMP Hibatullah</span></h2>\r\n      <p class="sdih-sec-sub">Fondasi utama dalam mencetak generasi Qur\\'ani yang berakhlak mulia</p>');
}
